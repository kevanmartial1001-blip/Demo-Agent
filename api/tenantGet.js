// /api/tenantGet.js
// Reads tenant row, stitches kb_json + kb_json_1..N, parses JSON, and verifies kb_sha256 when present.
...
function safeParseJson(raw) {
  ...
  try { return { ok: true, obj: JSON.parse(cleaned) }; }
  catch (e) { return { ok: false, err: String(e?.message || e), rawHead: cleaned.slice(0, 180), rawTail: cleaned.slice(-180) }; }
}

function sectionCounts(kbJson) {
  const s = kbJson?.sections || {};
  const out = {}; for (const [k, v] of Object.entries(s)) out[k] = Array.isArray(v) ? v.length : 0;
  return out;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (ok !== tenant) { return res.status(401).json({ ok: false, error: "invalid token" }); }
    ...
    if (values.length < 2) { return res.status(404).json({ ok: false, error: "sheet empty" }); }

    const headers = values[0].map((h) => String(h || "").trim());
    const idx = new Map(headers.map((h, i) => [h, i]));
    if (!idx.has("tenant_id")) { return res.status(500).json({ ok: false, error: "tenant_id column not found" }); }

    // find row
    let row = null;
    for (let r = 1; r < values.length; r++) {
      const line = values[r] || [];
      if ((line[idx.get("tenant_id")] || "") === tenant) { row = line; break; }
    }
    if (!row) { return res.status(404).json({ ok: false, error: "not found" }); }

    const get = (k) => (idx.has(k) ? (row[idx.get(k)] ?? "") : "");
    const kbSha = get("kb_sha256");
    ...
    const payload = {
      ...
      kb_sources: (() => { const raw = get("kb_sources_json"); try { return raw ? JSON.parse(String(raw)) : []; } catch { return []; } })(),
    ...
    if (kbJoined && kbJoined.length) {
      const sha = crypto.createHash("sha256").update(kbJoined).digest("hex");
      if (kbSha && kbSha !== sha) shaMismatch = true;
    ...
    if (debug) {
      ...
      if (kbSha) out._debug.kb_sha256_saved = kbSha;
      if (kbJoined) out._debug.kb_sha256_calc = crypto.createHash("sha256").update(kbJoined).digest("hex");
      if (shaMismatch) out._debug.kb_checksum_mismatch = true;
    }
